...
models = [
    ...
    (net.Classifier_2f_sub512_fc2,                      "2f_fc2_sub512_1out"),
...
def build_name(model_name, data_name, batch_size, seed = 1, use_norm = False, override = None):
    if override != None:
        return override
    if use_norm:
        data_name += "_norm"
...
def construct_flex_net():
    ...
    save_name = "flex_{c}c{cd}_{f}f{fd}_k{k}_s{s}_sub{p}".format(c=len(convs), cd=convstr, f=len(FCs)+1, fd=FCstr, k=kernels, s=strides, p=pool)
