...
def summary(model, input_size, batch_size=-1, device="cuda", gradients = False, Unit = 'MB', verbose = True):
    ...
    total_input_size = abs(np.prod(input_size) * batch_size * 4. / (1024 ** unit_mlt))
    
    total_output_size = abs(mlt_var * total_output * 4. / (1024 ** unit_mlt))  # x2 for gradients
